...
- **Sign-up form:**
  ...
  - **Client-side password strength meter added** (`app/ui/password-strength-meter.tsx`), as an alternative to hard server-side composition rules. Scoring via `zxcvbn-ts`, lazy-loaded on first keystroke and memoized at module scope so it never adds to the initial `/sign-up` bundle; scoring is debounced (~200ms) via `use-debounce`. Renders a 4-segment bar (`red-800`/`amber-800`/`blue-800`/`green-800`) with a "Password strength: Weak/Fair/Good/Strong" label, factoring in the user's email (`userInputs`) so email-derived passwords score lower. The empty gray track is always rendered (even before typing starts) to reserve layout space and avoid a jump when the meter first activates. Purely informational — never blocks or gates form submission; Clerk's 8-character minimum remains the only hard requirement.
...
- **Lighthouse audit performed on the landing page** (local production build): Performance 92–98, Accessibility 100, Best Practices 77, SEO 100. LCP ~2.4–3.3s, FCP ~0.9s, TTI ~3.1–3.8s (consistent across repeated runs, after the handshake-redirect fix below). Best Practices is capped by Clerk's `accounts.dev` third-party cookies/DevTools Issues entries — not fixable app-side without a custom Clerk domain.
- **`robots.txt` route added** (`app/robots.ts`) — previously missing entirely, so Lighthouse's SEO audit flagged the request as falling through to the app's HTML shell. Fixed SEO score from 92 → 100.
- **Cron routes bypass Clerk's `auth()` entirely** (`isCronRoute` in `proxy.ts`). `/api/keep-alive` and `/api/send-due` authenticate via their own `CRON_SECRET` bearer token and never carry a user session, so running them through Clerk's middleware was pure wasted work on every invocation.
- **Landing page (`/`) excluded from `proxy.ts`'s middleware matcher entirely**, fixing a ~1.7s Clerk "handshake" redirect chain on every first-time visitor (`GET /` → redirect to `*.clerk.accounts.dev/v1/client/handshake?...&__clerk_hs_reason=dev-browser-missing` → redirect back to `/?__clerk_handshake=<jwt>` → final render). Root cause: `clerkMiddleware()` performs this handshake before its callback even runs, for *any* route it's matched against — short-circuiting inside the callback (an earlier attempt) had no effect, since the wrapper itself already triggered it. The actual fix was changing the matcher's catch-all regex from `.*` to `.+` so a bare `/` (no characters after the leading slash) never matches, and `proxy.ts` never executes for that route at all. Landing page Performance jumped from 83 → 92–98 as a direct result. Trade-off: logged-in-but-not-onboarded users are no longer auto-redirected from `/` to `/onboarding` (they still hit that gate on `/dashboard`).
...
- **Auth form CapsLock warning layout fixed** — the warning was absolutely positioned (`absolute -bottom-5`) which required an oversize `mt-6` gap on the password error div below it to prevent overlap. Moved to normal flow (`mt-1 min-h-4`) so no space is reserved when inactive, with a single line reserved to prevent layout shift when the warning appears. `min-h-5` also removed from sign-in's form-level error div, which was creating excess whitespace between the password field and submit button.
...
- **`proxy.ts` matcher excluded `robots.txt` from Clerk protection** — the negative-lookahead extension list excluded `.html`/`.css`/`.js`/etc. from Clerk's route matcher but not `.txt`, so `/robots.txt` was being redirected to `/sign-in` instead of served. Added `txt` to the exclusion list.
- **`next.config.ts` had two conflicting exports** — `export default nextConfig` (empty) and a `module.exports = {...}` block (security headers) below it. Because the config loader transpiles to CommonJS, `module.exports` silently overwrote the `export default` object entirely; headers worked only because `nextConfig` was empty. Any future setting added to the `const nextConfig` object would have been discarded. Merged into a single export.
- **`/api/send-due` cron was unreachable** — it wasn't in `proxy.ts`'s `isPublicRoute` list, so Clerk's `auth.protect()` ran on it, misread the `CRON_SECRET` bearer token as an invalid Clerk JWT, and redirected to `/sign-in` (307) before the route handler's own auth check ever executed. The daily notification cron was silently failing on every invocation. Fixed by bypassing Clerk entirely for this route (see Performance & Code Quality). Note: the route is now reachable, but the pre-existing ±1-minute matching window vs. the daily-at-noon schedule (documented above under Notifications & PWA) still means most scheduled notification times won't fire — that's a separate, still-open issue.
- **`favicon.ico` was stale** — dated from before the RFS orbit-mark branding work, still showing the old icon. `scripts/generate-favicons.js` only ever generated the PNG sizes, never `.ico`. Extended it to also emit `favicon.ico` (via the new `png-to-ico` devDependency) so both stay in sync going forward.
