...
- **Lighthouse audit performed on the landing page** (local production build): Performance 83, Accessibility 100, Best Practices 77, SEO 100. LCP ~4.4s, FCP ~2.0s, TTI ~4.6s (consistent across repeated runs). Best Practices is capped by Clerk's `accounts.dev` third-party cookies/DevTools Issues entries — not fixable app-side without a custom Clerk domain.
