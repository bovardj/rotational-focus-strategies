...
**Auth form inputs:** On sign-in, use `type="text" inputMode="email"` (not `type="email"`) and omit `minLength` on the password — browser-native validation popups feel wrong when Clerk handles errors inline. On sign-up, `type="email"` is fine (format validation before submit is appropriate), but still omit `minLength` so Clerk returns `form_password_too_short` as an inline error. Common Clerk error codes: `form_identifier_not_found` (unknown email), `form_identifier_exists` (email taken), `form_param_format_invalid` (bad format), `form_password_too_short` / `form_password_size_too_small`, `form_password_pwned`, `form_code_incorrect`, `verification_expired`, `session_exists`. Use `React.SubmitEvent<HTMLFormElement>` for `onSubmit` handlers — `@types/react` 19.2+ deprecates `React.FormEvent` entirely ("FormEvent doesn't actually exist"; `onSubmit` is typed as `SubmitEventHandler`, not `FormEventHandler`). Use `React.ChangeEvent<HTMLInputElement>` etc. for `onChange` handlers, which are unaffected.
...
**Clerk middleware handshake redirect (`__clerk_hs_reason=dev-browser-missing`):** `clerkMiddleware()` performs its own cross-domain handshake redirect through `*.clerk.accounts.dev` (~1.7s) on *any* route matched by `proxy.ts`'s `config.matcher`, before the callback body even runs. An early `return NextResponse.next()` inside the callback does NOT prevent it — the route must be excluded from the matcher regex itself. Example: the landing page (`/`) needed no auth, so the catch-all matcher's capture group was changed from `.*` to `.+` (bare `/` has zero characters after the leading slash, so it no longer matches and `proxy.ts` never runs for it). Verify a fix with Lighthouse's `redirects` audit or a header diff — plain `curl` status codes won't reveal it, since the redirect chain is only visible to a real browser navigation.

**Cron routes vs. Clerk `auth.protect()`:** routes authenticated by a `CRON_SECRET` bearer token (`/api/keep-alive`, `/api/send-due`) must be listed in `isCronRoute` in `proxy.ts` and short-circuited *before* `auth()`/`auth.protect()` runs — Clerk misreads the Bearer token as an invalid session JWT and 307-redirects to `/sign-in` before the route handler's own auth check executes. Unlike the `/` case above, an early `return NextResponse.next()` inside the callback body *is* sufficient here (no matcher exclusion needed) — API routes don't trigger the handshake redirect the way page routes do.

**New Next.js-generated static routes** (`app/robots.ts`, `app/sitemap.ts`, etc.) need their extension added to `proxy.ts`'s matcher exclusion regex (see the `txt|webmanifest|...` list), or Clerk treats them as protected and redirects to `/sign-in`. Verify with `curl -I http://localhost:PORT/robots.txt` against a production build (`pnpm build && pnpm start`) after adding one.

**Local Lighthouse audits:** `pnpm build && pnpm start -p <port>` (never `pnpm dev` — unminified output skews scores), then `npx lighthouse http://localhost:<port>/ --output=json --chrome-flags="--headless=new --no-sandbox"`. Run it 3x — the first run immediately after a fresh build/server start is a noisy outlier (cold-start compilation inflates LCP/TTI); trust the steady-state numbers from repeat runs, not the first one.

**`/offline` and `/style-guide` are not in `proxy.ts`'s `isPublicRoute` list**, so unauthenticated visitors get redirected to `/sign-in` instead of seeing them — despite both being documented elsewhere as accessible pages (PWA offline fallback, internal design reference). Pre-existing on `main`, not caused by any of the middleware changes above. Add both to `isPublicRoute` if this needs fixing.

**Regenerating favicons after a logo change:** `scripts/generate-favicons.js` renders `app/icon.svg` to all PNG sizes (`android-chrome-*.png`, `apple-touch-icon.png`, `favicon-*.png`) *and* `favicon.ico` (via `png-to-ico`) — run `node scripts/generate-favicons.js` after any edit to `app/icon.svg`. `favicon.ico` previously wasn't part of this script and went stale for over a year without anyone noticing, since browsers rely on the App Router's `<link>` tags for the PNG/SVG icons and `.ico` is more of a legacy fallback.
