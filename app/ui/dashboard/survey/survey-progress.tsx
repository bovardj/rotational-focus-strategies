...
export default function SurveyProgress({ baselineDates, dailyDates }: SurveyProgressProps) {
  ...
  const baselineBlobs: DayBlob[] = Array.from({ length: 3 }, (_, i) => {
    ...
    return { date, done: date ? baselineSet.has(date) : false, phase: "baseline", index: i };
  });

  const dailyBlobs: DayBlob[] = Array.from({ length: 4 }, (_, i) => {
    const date = extrapolateDate(3 + i);
    return { date, done: date ? dailySet.has(date) : false, phase: "daily", index: i };
  });
  ...
  return (
    <div className="mb-6 max-w-2xl">
      ...
      <div className="flex flex-col w-full sm:w-auto sm:max-w-sm gap-2">
